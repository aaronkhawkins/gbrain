...
describe('embedding_signature stale semantics', () => {
  const current = 'v2|model=voyage%3Anew|dimensions=1024|column=embedding|preprocessing=text-document-v1';
  const prior = 'v2|model=voyage%3Aold|dimensions=1024|column=embedding|preprocessing=text-document-v1';
  ...
  test('R-4 GRANDFATHER: NULL signature is never stale', async () => {
    ...
    expect(await engine.countStaleChunks({ signature: current })).toBe(0);
    expect(await engine.sumStaleChunkChars({ signature: current })).toBe(0);
  });

  test('signature MISMATCH (model swap) is counted as stale', async () => {
    await seedEmbedded('drifted', 'abcde', prior); // 5 chars
    expect(await engine.countStaleChunks({ signature: current })).toBe(1);
    expect(await engine.sumStaleChunkChars({ signature: current })).toBe(5);
  ...
  test('MATCHING signature is not stale', async () => {
    await seedEmbedded('fresh', 'abcde', current);
    expect(await engine.countStaleChunks({ signature: current })).toBe(0);
    expect(await engine.sumStaleChunkChars({ signature: current })).toBe(0);
  });

  test('legacy v1 signature is unverified but not auto-invalidated', async () => {
    await seedEmbedded('legacy-v1', 'abcde', 'openai:old:1536');
    expect(await engine.countStaleChunks({ signature: current })).toBe(0);
    expect(await engine.invalidateStaleSignatureEmbeddings({ signature: current })).toBe(0);
  });

  test('invalidateStaleSignatureEmbeddings NULLs only mismatched; grandfathered + matching untouched', async () => {
    await seedEmbedded('old', 'abcde', prior); // mismatched → invalidate
    await seedEmbedded('legacy', 'fghij', null); // grandfathered → keep
    await seedEmbedded('new', 'klmno', current); // matching → keep

    const invalidated = await engine.invalidateStaleSignatureEmbeddings({ signature: current });
  ...
  test('invalidate is sourceId-scoped', async () => {
    ...
    await seedEmbedded('a', 'abcde', prior); // default
    await seedEmbedded('b', 'fghij', prior, 'other'); // other
    const n = await engine.invalidateStaleSignatureEmbeddings({ signature: current, sourceId: 'default' });
    expect(n).toBe(1);
