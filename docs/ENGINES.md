...
**Embedding is NOT in the engine.** The engine stores embeddings and searches by vector, but it doesn't generate embeddings. `src/core/embedding.ts` handles that. This is intentional: embedding is a provider concern (Ollama or OpenAI), not a storage concern. All engines share the same embedding service.
...
- `hybridSearch` falls back to keyword-only when the active embedding provider call fails
