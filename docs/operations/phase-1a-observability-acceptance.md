...
- [ ] AKH observer running with a separate `GBRAIN_HOME`, bind, and port
- [ ] Prometheus on akhserver01 scrapes both targets over Tailscale
...
2. `curl -s http://$AKH_BIND:$PORT/healthz` → different brain id
...
| Action | Expected |
|---|---|
| Stop personal observer only | Personal target DOWN; AKH remains visible |
