...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    // under the new language. The pages function intentionally uses the
    // final v124-safe body (no unbounded compiled_truth input) BEFORE the
    // UPDATE-to-self backfill. This closes the v122→v123 ordering hazard:
    // an oversized page cannot strand a non-English upgrade before v124
    // gets a chance to install the safe function. Databases already stamped
    // v123 still receive the same repair from v124.
    //
    // Skipped when the configured language is 'english' (trigger output
    // identical — re-tokenizing is wasted I/O).
